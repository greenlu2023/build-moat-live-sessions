...
def generate_token(url: str, db: Session) -> str:
    ...
    for attempt in range(MAX_RETRIES):
        nonce = str(attempt)
        data = f"{url}{nonce}".encode("utf-8")
        hash_digest = hashlib.sha256(data).digest()
        token = base62_encode(hash_digest)[:TOKEN_LENGTH]
        
        if not token_exists_in_db(db, token):
            return token
            
    raise RuntimeError(f"Could not generate a unique token after {MAX_RETRIES} retries.")
